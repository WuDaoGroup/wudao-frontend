<script>
    ...
    import { svcData } from '../../api/modelApi';
    ...
    function svc(percentOfTestData) {
        svcData(localStorage.filename + '_zscore_afterFilter.csv', percentOfTestData)
            .then((response) => {
                ...
                svcAnswerSheet =  [...svcAnswerSheet, theNewAns]
                errorSVC = true;
            })
            .catch(() => {
                errorSVC = false;
...
<div>
    <h1 class="mb-10 text-center text-blue-600">SVC</h1>
        ...
        <div>
            ...
            <div class="m-2 flex justify-center">
				<button
					...
					type="submit"
					on:click={svc( percentOfTestData )}>Submit</button
				>
			</div>
        </div>
        <div>
            <Accordion>
                <AccordionItem title="Answers">
                    {#if errorSVC}
                        {#each svcAnswerSheet as { accuracyOfTestData, Accuracy, 
