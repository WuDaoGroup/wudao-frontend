<script>
	...
	const fillDataOption = ['请选择', '均值填充', '中位数填充'];
	const defaultChoice = fillDataOption[0];
	let fillType = { key: defaultChoice };
	let code = [];
	...
	function confirmOriginZscore() {
		console.log(fillType.key);
		confirmOriginZscoreApi(localStorage.filename, fillType.key).then((response) => {
			if (response.status == 200) {
				console.log('response_data:', response.data);
				code.push(response.data.code);
...
<div class="grid grid-cols-5 gap-4">
	...
	<Button on:click={receiveBasicData} kind="tertiary">数据标准化</Button>
</div>
{#if showTable == true}
...
<div class="grid grid-col-5">
	...
	<div class="col-span-3">
		<Select inline labelText="选择填充方式" bind:selected={fillType.key}>
			{#each fillDataOption as p}
